...
@Component
public class InstancesUserHandlerImpl extends ReportBaseHandler implements ReportHandler {
    ...
    public Status handle(Status deviceStatus, Device device, UserInfoBean userInfo, WarningService warningService, ReportItem reportItem, Status monitorType) {
        ...
        //发送SysLog日志
        SysLogUtil.sendLog(config, device.getData().getDeviceIP(), device.getData().getDeviceHostname(), "用户储存库", "用户储存库[" + reportItem.getItem() + "][" + jSONObject.getString("ports") + "][" + jSONObject.getString("message") + "]");
        ...
        if (reportItem.getStatus() == 0) {
            ...
            warningEvent.setEventDetail("用户储存库[" + reportItem.getItem() + "][" + jSONObject.getString("ports") + "][" + jSONObject.getString("message") + "]");
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("InstancesUserHandler return {}" + monitorItemType);
        }
    ...
    @PostConstruct
    public void init() {
        ReportHandlerPools.getInstance().registerHandler(MonitorItemEnum.INSTANCES_USER, this);
    }
}
