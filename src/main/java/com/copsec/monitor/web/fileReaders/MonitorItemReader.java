...
public class MonitorItemReader extends BaseFileReader<MonitorItemBean> {
    ...
    public void getData(String filePath) throws CopsecException {
        ...
        if (!ObjectUtils.isEmpty(list)) {
            list.stream().
                    filter(d -> !ObjectUtils.isEmpty(d) && d.split(Resources.SPLITER, -1).length == 4)
                    .forEach(item -> {
...
//                        bean.setMonitorType(MonitorTypeEnum.valueOf(dataList[3]));
                        if (CommonUtils.isJSONValid(dataList[3]) && !"".equals(dataList[3])) {
                            bean.setItem(JSON.parseObject(dataList[3], CertConfig.class));
                        } else {
                            bean.setItem(dataList[3]);
                        }
