...
function page(id, url) {
    ...
    var aoColumns = [//渲染每一列，其实就是配置表头和数据对应显示到哪一列中
        {
            "mData": "eventId",
            "sClass": "text-center",
            "sTitle": '选择',
    ...
    oTable = $("#" + id).dataTable({//注意#infoTable是需创建为dataTable的表格,使用jQuery选择器
        ...
        "fnServerParams": function (aoData) {
            ...
            var eventTime = $("#eventTime").val().split("&");
            aoData.push({
                "name": "start",
                "value": eventTime[0]
            });
            ...
            var dealTime = $("#dealTime").val().split("&");
            aoData.push({
                "name": "dStart",
                "value": dealTime[0]
            });
...
jQuery(function () {
    ...
    var $state = $("#warningHistoryTable thead input[type='checkbox']");
...
function deleteData(bean) {
    ...
    var $message = $('<h2 style="text-align: center">确认删除[' + bean.deviceName + ']此条历史？</h2>');
...
function deleteCheck() {
    ...
    var $message = $('<h2 style="text-align: center">确认删除所选历史？</h2>');
    $("#message").find(".modal-body").html($message);
    addButton("message", "系统提示", $confirmButton);
